"""
Step 6: SAR-Only Baseline (Reviewer 3 Point 1)
Addresses: "No multi-modal fusion experiments to validate foundation claim"

This script trains a 2-channel DeepLabV3+ model using ONLY Sentinel-1 SAR data:
✅ SAR VV and VH polarizations as INPUT CHANNELS
❌ NO Optical data

Purpose: Establish SAR-only baseline for fusion comparison

Expected Outcome:
- SAR-only F1: ???%
- Compare to optical-only (99.17% F1)
- Compare to fusion (10ch, pending)
"""
...
class SAROnlyDataset(Dataset):
    """
    S1S2-Water dataset with ONLY SAR data (VV, VH)
    """
    ...
    def __init__(self, data_dir="dataset/s1s2_water_minimal", mode='train',
        ...
        if tiles_per_sample is None:
            ...
            # Each sample loads full image temporarily (~1-2GB), so fewer samples loaded at once
            tiles_per_sample = 50 if mode == 'train' else 25

        print(f"\n{'='*70}")
        print(f"SAR-ONLY DATASET ({mode.upper()})")
        print(f"{'='*70}")
    ...
    def _load_sample_tiles(self, sample_id, num_tiles):
        ...
        try:
            ...
            # SAR: 12200×12200, Optical: 10980×10980
            ...
            for _ in range(num_tiles):
                ...
                tile_img = s1_normalized[:, y:y+self.tile_size, x:x+self.tile_size]
                ...
                self.tiles.append({
                    'image': tile_img,  # Shape: (2, 256, 256)
    ...
    def apply_augmentation(self, image, mask):
        """Apply augmentation to SAR data"""
        if not self.augment:
            return image, mask
        ...
        # 4. Speckle noise (SAR-specific, p=0.3)
    ...
    def __getitem__(self, idx):
        ...
        # Get 2-channel SAR image
        image = tile['image'].copy()  # (2, H, W)
...
def create_model(num_classes=1, in_channels=2):
    """
    Create DeepLabV3+ model with 2-channel SAR input
    """
...
def train_sar_only():
    """
    Train 2-channel SAR-only model
    """

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    print("\n" + "="*80)
    print("SAR-ONLY BASELINE EXPERIMENT (Reviewer 3 Point 1)")
    print("="*80)
    print("Configuration:")
    print("  [OK] 2-channel input (SAR VV + VH)")
    print("  [OK] Standard loss (Dice + Focal + BCE)")
    print("  [OK] FULL 45 training samples")
    print()
    print("Purpose:")
    print("  Establish SAR-only baseline for fusion comparison")
    print()
    print("Expected Outcome:")
    print("  Optical-only (8ch): 99.17% F1")
    print("  SAR-only (2ch): ???% F1")
    print("  Fusion (10ch): ???% F1 (pending)")
    print("="*80 + "\n")
    ...
    # Load datasets
    train_dataset = SAROnlyDataset(mode='train', augment=True)
    val_dataset = SAROnlyDataset(mode='val', augment=False)
    test_dataset = SAROnlyDataset(mode='test', augment=False)
    ...
    for epoch in range(max_epochs):
        ...
        if val_f1 > best_val_f1:
            best_val_f1 = val_f1
            torch.save(model.state_dict(), 'manuscript/best_sar_only_2ch.pth')
    ...
    model.load_state_dict(torch.load('manuscript/best_sar_only_2ch.pth'))
    model.eval()
    ...
    print(f"SAR-only (2ch):     {test_f1*100:.2f}% F1")
    print(f"Difference:         {(test_f1 - optical_only_f1)*100:+.2f}pp")
    print("="*80 + "\n")

    # Save results
    results = {
        'experiment': 'SAR-Only Baseline',
        'model': '2-channel DeepLabV3+ (ResNet-50)',
        'input_channels': 'SAR VV + VH',
        ...
        'comparison': {
            ...
            'sar_only_f1': test_f1,
            'difference': test_f1 - optical_only_f1
        },
        'training_history': history,
        'timestamp': datetime.now().isoformat()
    }

    with open('manuscript/sar_only_results.json', 'w') as f:
        json.dump(results, f, indent=2)

    print("Results saved to: manuscript/sar_only_results.json")
    print("Model saved to: manuscript/best_sar_only_2ch.pth")
    print("="*80 + "\n")
...
if __name__ == '__main__':
    print("\n" + "="*80)
    print("SAR-ONLY BASELINE EXPERIMENT")
    print("="*80)
    print("This experiment establishes SAR-only performance baseline")
    print()
    ...
    results = train_sar_only()

    print("\n" + "="*80)
    print("[OK] SAR-ONLY BASELINE EXPERIMENT COMPLETE")
    print("="*80)
    print("Next steps:")
    print("  1. Review results in: manuscript/sar_only_results.json")
    print("  2. Run SAR+Optical fusion experiment (step7)")
    print("  3. Compare all three: Optical-only, SAR-only, Fusion")
    print("="*80 + "\n")
